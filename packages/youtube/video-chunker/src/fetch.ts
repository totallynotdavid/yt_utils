...
import type { FetchProgress } from "./events.ts";
import { eachLine } from "./lines.ts";
import type { FetchedSource } from "./media.ts";
...
function detectJsRuntime(): string | null {
  for (const runtime of ["deno", "node", "bun", "quickjs"]) {
    if (Bun.which(runtime) !== null) return runtime;
  }
  return null;
}
...
export async function fetchVideo(url: string, opts: FetchOptions): Promise<FetchedSource> {
  ...
  const proc = Bun.spawn(args, { stdout: "pipe", stderr: "pipe", signal: opts.signal });
